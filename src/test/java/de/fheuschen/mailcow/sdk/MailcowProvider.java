...
public class MailcowProvider {
  ...
  public static final String MCJ_API_KEY_ENV = "MCJ_KEY", MCJ_API_URL_ENV =
                                                              "MCJ_URL";
  ...
  static {
    ...
    if (System.getenv(MCJ_API_URL_ENV) != null)
      API_URL = System.getenv(
          MCJ_API_URL_ENV); // Default test server is only accessible from
  ...
  public static Mailcow getMailcowInstance() {
    if (m == null) {
      m = new APIBuilder().setServerURL(API_URL).setApiKey(API_KEY).build();
    }
    return m;
  }
