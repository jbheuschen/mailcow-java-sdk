...
public abstract class BaseClient {
  ...
  public void initialize(Mailcow m) { this.m = m; }

  /**
   * This method performs the necessary authentication on the WebTarget object
   * (i.e., adding api keys or similar)
   * @param t the target
   ...
   * @return
   */
  public Response performGetRequest(Endpoint<?> endpoint,
                                    Map<String, String> params)
      throws MailcowException {
    WebTarget t = server.path(endpoint.getEndpointUrl());
    for (String key : params.keySet())
      t.queryParam(key, params.getOrDefault(key, ""));
    return throughout(
        this.doAuthentication(t.request(MediaType.APPLICATION_JSON)).get());
  }
   ...
   * @return
   */
  public <T extends MailcowModel>
      T performGetRequest(Endpoint<T> endpoint, Map<String, String> params,
                          Class<T> clazz, String id) throws MailcowException {
    WebTarget t =
        server.path(endpoint.getEndpointUrl() + ((id == null) ? "" : id));
    if (params != null)
      for (String key : params.keySet())
        t.queryParam(key, params.getOrDefault(key, ""));
    return parseToItem(
   ...
   * @return
   */
  public <T extends MailcowModel> Response performPostRequest(
      Endpoint<T> endpoint, RequestType type, Map<String, String> params,
      OMailcowModel<T> om, String... items) throws MailcowException {
    if (endpoint.writing(type) && !checkReadOnly())
      return null;
    ...
    if (params != null)
      for (String key : params.keySet())
        t.queryParam(key, params.getOrDefault(key, ""));
    return throughout(
        this.doAuthentication(t.request(MediaType.APPLICATION_JSON))
            .post(Entity.json(g.toJson(om.wrap(items)))));
  }
   ...
   * @return
   */
  public Response performPostRequest(Endpoint<?> endpoint, RequestType type,
                                     Map<String, String> params,
    ...
    return throughout(
        this.doAuthentication(t.request(MediaType.APPLICATION_JSON))
            .post(Entity.json(g.toJson(body))));
  }
   ...
   * @return
   */
  public <T extends MailcowModel> Collection<T>
  performMultiGetRequest(Endpoint<T> endpoint, Map<String, String> params,
                         Class<T> clazz, String id) throws MailcowException {
    WebTarget t =
        server.path(endpoint.getEndpointUrl() + ((id == null) ? "" : id));
    if (params != null)
      for (String key : params.keySet())
        t.queryParam(key, params.getOrDefault(key, ""));
    Response r = throughout(
        this.doAuthentication(t.request(MediaType.APPLICATION_JSON)).get());
    return g.fromJson(r.readEntity(String.class),
   ...
   * @return
   */
  public boolean performDelete(MailcowModel m, Map<String, Object> params,
                               ODeletePacket p) throws MailcowException {
    if (!checkReadOnly())
      return false;
    WebTarget t = server.path(m.getEndpoint().get(RequestType.DELETE));
    return throughout(
               this.doAuthentication(t.request(MediaType.APPLICATION_JSON))
                   .post(Entity.json(p.getObjectForSerialization())))
               .getStatus() < ERROR_THRESHOLD;
  }

  /**
   * Performs a check whether the API is set to read-only. If throwOnWrite is
   * set to true and the api is ro, this will throw an
  ...
  public boolean checkReadOnly() throws UnsupportedAPIActionException {
    if (m.isReadOnly() && m.isThrowOnWrite())
      throw new UnsupportedAPIActionException(
          "Cannot perform write action on read-only API!");
    return !m.isReadOnly();
  }

  /**
   * Checks for a successful by trying to fetch a list of domains (as most
   * servers will have at least one domain configured and - even if that's not
   * the case - there will always be an empty list of domain that's returned).
   * @return true/false
   */
  public boolean connectionSuccessful() {
    try {
      Collection<Domain> d = new DomainBuilder().fetchAll(m);
      return d != null;
   ...
   * servers will have at least one domain configured and - even if that's not
   * the case - there will always be an empty list of domain that's returned).
   * @param throwException if set to true and an exception occurs, it will be
   *     rethrown (i.e., you can handle it yourself).
   * @throws MailcowException if throwException is true
   * @return true/false
   */
  public boolean connectionSuccessful(boolean throwException)
      throws MailcowException {
    try {
      Collection<Domain> d = new DomainBuilder().fetchAll(m);
      return d != null;
   ...
   * @param <T> the type
   * @return the item
   * @throws ItemNotFoundException if the json does not contain an item of that
   *     class; thus, it is likely that there was no item found.
   */
  protected <T extends MailcowModel> T parseToItem(String json, Class<T> clazz)
  ...
  protected void handleStatusCode(int statusCode) throws MailcowException {
    switch (statusCode) {
    case 401:
      throw new AuthenticationFailedException("Got 401 Authentication failed!");
    case 403:
      throw new AuthenticationFailedException("Got 403 Authentication failed!");
    case 404:
      throw new UnsupportedAPIActionException("Got 404 Not found!");
    case 500:
      throw new MailcowException("Got 500 Internal Server Error!");
    default:
    }
  }
